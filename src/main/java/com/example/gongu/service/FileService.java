...
import com.example.gongu.mapper.FileMapper;
...
public class FileService {
    ...
    public void remove(Long applyNumber){
        ...
        List<FileDto> fileList = findList(applyNumber);
        ...
        fileMapper.delete(applyNumber);
    }
    //    파일 리스트 조회
    public List<FileDto> findList(Long applyNumber){
        if (applyNumber == null) {
            throw new IllegalArgumentException("게시물 번호 누락!!");
        }

        return fileMapper.selectList(applyNumber);
    }
...
//        이미지 파일인 경우에만 썸네일을 저장해야한다.
     ...
     * @throws IOException
     */
    public void registerAndSaveFile(List<MultipartFile> files, Long applyNumber) throws IOException{
