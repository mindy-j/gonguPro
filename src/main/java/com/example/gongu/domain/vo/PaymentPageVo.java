...
public class PaymentPageVo {
    ...
    public PaymentPageVo() {}

    public PaymentPageVo(int total, PaymentCriteria paymentCriteria) {
        this(5, total, paymentCriteria);
    }

    public PaymentPageVo(int pageCount, int total, PaymentCriteria paymentCriteria) {
...
//        ceil() : 올림 처리
        this.endPage = (int)(Math.ceil(paymentCriteria.getPage() / (double)pageCount) * pageCount);
        this.startPage = endPage - pageCount + 1;


//        게시글 전체 수로 실제 마지막 페이지를 구한다.
        this.realEnd = (int)Math.ceil((double)total/paymentCriteria.getAmount());
